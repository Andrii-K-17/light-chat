...
<template>
  ...
  >
    ...
    <aside
      ...
      class="flex flex-shrink-0 flex-col overflow-hidden border-r border-slate-200/80 bg-white/90 transition-[width] duration-200 ease-out dark:border-slate-800/80 dark:bg-slate-900/80"
    >
      <!-- Full sidebar -->
      <div
        v-if="isSidebarOpen"
        class="flex h-full flex-col pb-3"
      ...
      >
        ...
        <ul class="no-scrollbar flex-1 space-y-1 overflow-y-auto px-2 pb-3">
          <SidebarItem v-for="chat in filteredChats" :key="chat.id" :chat="chat" />
          <li
    ...
    <div
      v-if="isSidebarOpen"
      class="relative z-10 w-1 flex-shrink-0 cursor-col-resize bg-slate-200/70 hover:bg-emerald-500 dark:bg-slate-800 dark:hover:bg-emerald-500"
    ...
    <!-- Main panel -->
    <main
      class="relative flex min-w-0 flex-1 flex-col bg-slate-50/10 transition-colors dark:bg-slate-900/30"
    >
      ...
      >
        ...
        <div class="relative flex flex-col items-center">
          ...
          <h1 class="text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">
            Welcome to
            <span class="text-emerald-500 dark:text-emerald-400">LightChat</span>
          </h1>
          ...
          <button
            @click="showNewChat = true"
            class="mt-6 rounded-xl bg-emerald-600 px-5 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-600/15 transition-all hover:cursor-pointer hover:bg-emerald-700 hover:shadow-emerald-600/25 active:scale-[0.98] dark:bg-emerald-600 dark:hover:bg-emerald-500"
          >
            New Chat
          </button>
        </div>
